    ...
    <div class="flex justify-center items-center h-screen bg-gray-100">
        <div class="w-full max-w-md p-6 bg-white rounded-lg shadow-md">
            <transition name="fade">
                <div v-if="successMessage" class="mt-6 p-2 text-center text-green-600">{{ successMessage }}</div>
            </transition>
            <transition name="fade">
                <div v-if="errorMessage" class="mt-6 p-2 text-center text-red-600">{{ errorMessage }}</div>
            </transition>
            ...
            <form @submit.prevent="resetPassword" class="space-y-6">
                <div>
                    ...
                    <input v-model="matricNumber" id="matricNumber" type="text" placeholder="Matric Number" class="input"
                        required />
                </div>

                <div>
                    <label for="securityQuestion" class="block text-sm font-medium text-gray-700">Security Question</label>
                    <select v-model="securityQuestion" id="securityQuestion" class="input" required>
                ...
                <div>
                    ...
                    <input v-model="securityAnswer" id="securityAnswer" type="text" placeholder="Security Answer"
...
const resetPassword = async () => {
    ...
    try {
        const response = await axios.post('http://localhost:8800/api/v1/auth/resetpw', {
