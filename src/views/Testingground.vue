<template>
    <div class="container mx-auto p-4">
          ...
          <tbody>
            <tr v-for="course in courses" :key="course.coursecode">
              ...
              <td class="py-2 px-4 border">
                <select v-model="course.grade" @change="updateCGPA" class="border p-1">
        ...
        <table class="min-w-full bg-white border">
          ...
          <tbody>
            <tr v-for="(analysis, grade) in gradeAnalysis" :key="grade">
              ...
              <td class="py-2 px-4 border">{{ analysis.courseCodes.join(', ') }}</td>
      ...
      <div class="mt-8">
        <h2 class="text-xl font-semibold mb-2">Current CGPA: {{ cgpa.toFixed(2) }}</h2>
      </div>
    </div>
  </template>
