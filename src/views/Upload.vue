<template>
    <div class="container mx-auto p-8 font-display">
        ...
        <div v-if="imageUrl" class="mt-4 flex flex-col gap-2">
            ...
            <input type="text" v-model="title" placeholder="Image Title" class="border border-gray-400 p-2" />
            <input type="text" v-model="description" placeholder="Description" class="border border-gray-400 p-2" />
            <input type="text" v-model="tags" placeholder="Tags" class="border border-gray-400 p-2" />
            <input type="text" v-model="features" placeholder="Featuring:" class="border border-gray-400 p-2" />
            <button @click="submitData" class="mt-2 bg-blue-500 text-white px-4 py-2 disabled:opacity-50"
                :disabled="isSubmitting">
                {{ isSubmitting ? 'Submitting...' : 'Submit' }}
            </button>
        </div>

        <div v-if="Object.keys(validationErrors).length > 0" class="mt-4 bg-red-100 p-4 rounded">
            <ul>
        ...
        <div v-if="submissionMessage" class="mt-4 p-4 rounded blink"
            :class="{ 'bg-green-100 text-green-700': messageType === 'success', 'bg-red-100 text-red-700': messageType === 'error' }">
            {{ submissionMessage }}
        </div>
    </div>
</template>
